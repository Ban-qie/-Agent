...
### 2026-09-25 medium 补丁部署续作（最新）

- [x] 用户回复“好的，继续任务”后，冻结本地提交 `280045f96119391fb06f74a01918e89ea6a78d93`，未推送。源码包和镜像均有 SHA-256；基于旧生产镜像离线增加三个代码文件的补丁层，依赖与前端不变。
- [x] 新镜像 `sha256:4369eda4c5baafa569817ae749808ac584b5eda747c05d0e3a518ce207da74a1` 在本地及香港目标断网容器均通过 6 项真实 SDK/假 HTTP 回归。
- [x] 首次部署的检查脚本将 PostgreSQL memoryview 转为含内存地址的字符串，误报旧归档指纹变化；自动回滚成功。连续读同一行复现了检查缺陷，改为二进制内容序列化并校验原始账目 SHA-256。失败日志和两版脚本保留，不删除字段或放宽断言。
- [x] 第二次部署成功：仅重建 app，维护窗口 8.89 秒；其余三个服务未重建，四容器 healthy、重启计数 0。9 张持久表的内容指纹完全一致，6 条 usage/3 条 unknown 保留，旧账目原始哈希匹配。
- [x] 部署前新增数据库备份并校验 pg_restore 目录；备份复制至本机并验证 SHA-256。旧镜像保留为 `ecominsight:v4-rollback-s06c-medium-fix1-predeploy`，旧标签/volume 不删除。
- [x] 本机直连公网 healthz、readyz、匿名 auth/status 均 200，readiness 全部通过；本轮真实模型请求 0、新增费用 0。
- [ ] 真实 Attempt4：新 request ID `v4-s06c-smoke-20260925-04`，拟仅提交原固定问题一次、最多3次内部调用、自动重试0、新增费用上限0.10元、项目累计上限10元。等待单独明确授权，不执行旧 smoke 脚本。
