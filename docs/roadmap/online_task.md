...
更新日期：2026-09-23  
当前阶段：V4 稳定上线准备  
当前停留点：香港生产实例和 `ecominsight.cn` 已就绪，V4-S01—S03、V4-S04a 已通过，下一步为 V4-S04b 双应用实例 20+20 HTTP harness；尚未修改 DNS、部署远程应用或开放网站。香港目标无需且不能办理中国大陆 ICP 备案。
...
- [ ] V4-S04：两用户并发、异常输入、重复请求、越权、取消、重启、存储失败和预算耗尽组合验收；至少 20 个正常 stub 与 20 个非法/重复/越权样本。
- [ ] V4-S05：可审阅发布树、秘密扫描、回滚包、迁移说明和演示材料。
...
6. [ ] 完成 V4-S05 固定发布包、秘密扫描和回滚演练，生成可传到香港服务器的同一发布制品。
...
- [ ] 当前下一步：V4-S05a 发布 manifest。先从固定工作树生成可重建发布树并记录文件、依赖、许可证和配置 hash，再依次执行秘密扫描、同构回滚演练和演示材料验收。
