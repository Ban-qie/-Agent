...
- [x] V4-S05a-d 已完成：最终发布 manifest、秘密扫描、回滚演练和演示材料均已通过；最终包 commit 为 `569c53d5`。
- [x] V4-S05a-d：最终发布包来源 `569c53d5`，归档、镜像 digest、锁依赖、Olist 快照和 Portfolio 均有证据；秘密扫描命中 0；回滚恢复、旧代码兼容边界、失败后入口关闭和 3–5 分钟演示脚本均通过。真实 Qwen 0，账目 hash 不变。
