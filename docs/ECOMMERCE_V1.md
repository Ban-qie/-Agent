...
本地源码版本 `v0.2.0`，基于 V0 `v0.1.0`，适用于 Windows 本机单用户和 local 工作区。V1 使用 LangGraph `1.2.11` 组织七个确定性业务节点，不调用模型。V0 的 Qwen + 单 AnalystAgent 路径保留，且仍为默认模式。
...
原始 CSV、派生 Parquet、工作区、密钥和费用账目不在源码中。数据许可、固定指纹及取得方式见 [来源与许可](../THIRD_PARTY_NOTICES.md) 和 V0 指南。V1 本身无需模型凭据或费用账目才能执行固定只读分析，但已有项目的费用账目必须原样保留。
...
若需源码级回退，停止服务、备份私有数据并保留本地改动，在独立目录检出 `v0.1.0`；V1 固化点为本地 `v0.2.0`。不要 reset 当前工作区或把 V1 文件覆盖为 V0 的 `session_state.json`。标签未推送，远端无法使用本地标签直到另行发布。

## 验证范围

V1-15 已完成 264 项相关后端、15 项前端、类型检查、构建、13 个 V1 浏览器场景、9 个原始 CSV 复算场景及两项真实 V0 对照。本次固化只做文件/版本/完整性检查，不重复业务或付费验收。摘要见 [RELEASE_V1.md](RELEASE_V1.md) 和 [validation/V1-summary.json](validation/V1-summary.json)。
