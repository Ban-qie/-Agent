...
### V1-R03 分组切换继承规则（2026-09-18）

- 排序与Top N属于分组展示策略。用户显式切换region/day/month时，不继承旧维度的sort/top_n；只采用本次明确指定的新排序和Top N。
- 期间、地区与指标仍遵循明确替换/未指定继承；同维度追问继续继承原排序；原父节点不可变。
- 两期日趋势分别展示绝对日期，不将不同月份的日期键强行配成差额排名。地区差额排名继续按两期同地区键比较。
- 本轮真实worker测试：从地区差额Top3切换SP日趋势，当前期28天、基准期31天完整，日期有序，无旧ranking；65项相关测试通过。真实Qwen完整页面链路待R04，不冒充已验收。
