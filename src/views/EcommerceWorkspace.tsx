...
import { AnalysisNode, AnalysisResponse, MetricResult, ResultRow, errorMessage, examples, formatAmount, periodLabel,
    resultRows, salesSpec } from './ecommerce';

function SalesChart({ rows }: { rows: ResultRow[] }) {
    ...
    useEffect(() => {
        ...
        setError(false);
        embed(host.current, salesSpec(rows), { actions: false, renderer: 'svg' }).then(result => {
    ...
    return <Box sx={{ overflowX: 'auto' }}>
        ...
        <Box ref={host} role="img" aria-label="商品金额柱状图，与结果表使用相同数据"
...
export function EcommerceResults({ response }: { response: AnalysisResponse }) {
    ...
    const result = failed ? response.partial_result : response.result;
    const conditions = response.conditions || result?.conditions;
    ...
    return <Stack spacing={2} aria-live="polite">
        ...
        {failed && <Alert severity="error">{errorMessage(response.error?.code || response.result?.error?.code)}{response.partial_result && ' 以下仅为已验证的部分结果，分析闭环未完成。'}</Alert>}
        ...
        {result?.state === 'outside_coverage' ? <Alert severity="warning">日期超出快照观测范围，未裁剪日期或放宽条件；没有生成图表。</Alert> : result && <>
            ...
            {rows.length > 0 && <>
                {truncated && <Alert severity="warning">分组已截断：每期最多显示 100 组。图表与表格仅含显示的分组，上方总计仍覆盖全部选定记录。</Alert>}
                <Paper variant="outlined" sx={{ p: 2 }}><Typography variant="subtitle1">商品金额对照</Typography>
                    <Typography variant="caption" color="text.secondary">宽分组图和结果表可横向滚动查看。</Typography><SalesChart rows={rows} /></Paper>
        ...
        {result && <Box component="details" sx={{ p: 2, border: '1px solid', borderColor: 'divider', borderRadius: 1, overflowWrap: 'anywhere' }}>
            ...
            <Typography variant="caption" component="p">快照：{conditions?.snapshot_id} · 指标版本：{conditions?.metric_version}</Typography>
...
export function EcommerceWorkspace() {
    ...
    const last = useRef<{ question: string; id: string }>();
    const selectNode = (node: AnalysisNode) => {
        last.current = { question: node.user_question, id: node.request_id };
    ...
    const loadWorkspace = async (restore = false) => {
        ...
        if (state.schema_version !== 1 || !Array.isArray(state.nodes)) throw new Error();
        setNodes(state.nodes); setLoaded(true); setRestoreError(false);
        const node = restore ? state.nodes.at(-1) : state.nodes.find((n: AnalysisNode) => n.request_id === selectedId);
    ...
    useEffect(() => {
        ...
        fetchWithIdentity('/api/ecommerce/workspace').then(async res => {
            ...
            if (state.schema_version !== 1 || !Array.isArray(state.nodes)) throw new Error();
            if (active) {
                setNodes(state.nodes); setLoaded(true);
    ...
    const submit = async (event: React.FormEvent) => {
        ...
        if (last.current?.question !== question) last.current = { question, id: crypto.randomUUID() };
        ...
        try {
            ...
            const data = await res.json();
            if (!['success', 'empty_result', 'outside_coverage', 'failed', 'clarification_required', 'running', 'interrupted'].includes(data.state)) throw new Error();
            setResponse(data);
            const saved = await fetchWithIdentity('/api/ecommerce/workspace');
            if (!saved.ok) throw new Error();
            const state = await saved.json();
            if (state.schema_version !== 1 || !Array.isArray(state.nodes)) throw new Error();
            setNodes(state.nodes); setRestoreError(false);
            const node = state.nodes.find((n: AnalysisNode) => n.request_id === last.current?.id);
            if (node) setResponse(node.response);
    ...
    return <Box sx={{ height: '100dvh', overflow: 'auto', bgcolor: 'background.default' }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '300px minmax(0, 1fr)' }, gap: 3, p: { xs: 2, md: 3 }, maxWidth: 1500, mx: 'auto' }}>
            <Paper component="aside" variant="outlined" sx={{ p: 2, alignSelf: 'start' }}>
                ...
                <Stack spacing={1} sx={{ mt: 1 }}>{examples.map(text => <Button key={text} disabled={busy || !loaded} variant="text" sx={{ textAlign: 'left', whiteSpace: 'normal', justifyContent: 'flex-start' }} onClick={() => { last.current = undefined; setQuestion(text); setResponse(undefined); setNetworkError(false); }}>{text}</Button>)}</Stack>
                ...
                <Stack spacing={1}>{[...nodes].reverse().map(node => <Button key={node.node_id} disabled={busy || !loaded}
                    ...
                    {node.user_question} · {({ success: '完成', empty_result: '空结果', outside_coverage: '覆盖外', failed: '失败',
                        clarification_required: '待澄清', running: '运行中', interrupted: '中断' } as Record<string, string>)[node.response.state] || '未知状态'}
                </Button>)}</Stack>
            </Paper>
            <Stack component="main" spacing={2} sx={{ minWidth: 0 }}>
                ...
                <Box component="form" onSubmit={submit}>
                    ...
                    {['failed', 'interrupted'].includes(response?.state || '') && <Button type="button" disabled={busy} sx={{ mt: 1, ml: 2 }}
                        onClick={event => { last.current = undefined; void submit(event); }}>
                        故障已排除，发起新请求
                    </Button>}
                </Box>
