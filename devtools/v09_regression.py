"""Reproducible offline V0 acceptance suite with structured test evidence."""
import json
...
def main():
    ...
    status = pytest.main([
        'tests/backend/ecommerce', 'tests/backend/agents/test_qwen_probe.py',
        ...
        '-q', '-p', 'no:cacheprovider', '--basetemp', str(ROOT / '.local' / ('pytest-' + uuid.uuid4().hex)),
    ], plugins=[evidence])
    report = {'stage': 'V0-9', 'mode': 'offline: provider mocked; real fixed workers and OS limits where tested',
              'exit_code': int(status), 'seconds': round(time.monotonic() - started, 3),
    ...
    (ROOT / 'docs/verification/V0-9-regression.json').write_text(json.dumps(report, indent=2), encoding='utf-8')
    return status
